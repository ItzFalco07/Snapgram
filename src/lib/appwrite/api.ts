//update createEmailSession from createSession if error
...
export async function updateUser(user: IUpdateUser) { 
  ...
  try {
    ...
    if (hasFileToUpdate) {
      ...
      // Get new file url
      const fileUrlPromise = getFilePreview(uploadedFile.$id); // Note: This might be a promise
      const fileUrl = await fileUrlPromise; // Await the promise to get the URL
      if (!fileUrl) {
        await deleteFile(uploadedFile.$id);
        throw Error;
      }
